...
def pet_filter(letter="a"):
    ...
    for n in range(len(pets)):
        if letter in list(pets[n]):
            pets_with_letter.append(pets[n])

    return pets_with_letter
...
def best_letter_for_pets():
    ...
    A_list = list(the_alphabet)

    pet_letter_number = -1

    for n in range(len(A_list)):
        new_letter_number = len(pet_filter(letter=A_list[n]))
        if new_letter_number > pet_letter_number:
            pet_letter_number = new_letter_number
            most_used_letter = A_list[n]
    ...
    for i in range(3):
        for l in range(3,8):
            ...
            if r.status_code is 200:
                filler_text_dictionary.setdefault(l, [])
                filler_text_dictionary[l].append(r.text)
...
def fast_filler(number_of_words=200):
    ...
    with open('dict_racey.json', 'w') as dict_racey:
        json.dumps(make_filler_text_dictionary(), dict_racey)
    # print(dict_racey)
